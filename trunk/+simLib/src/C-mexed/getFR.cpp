...
struct _simRes simMex(bool isSystematic, unsigned int kl, unsigned int sSize, double pl, double per, unsigned int iterations, unsigned int seed, bool pruneZeros, unsigned int nl)
{
    srand(seed);
    ...
    for (unsigned int it = 0; it < iterations; it++)
    {
        ...
        for ( unsigned int nn_ = 0; nn_ < nl; nn_++ ) {
...
{   
    ...
    unsigned int nl = (unsigned int) mxGetScalar(prhs[9]);
    ...
    if ( q == 2 )
        tmpRVal = simMex<Encoder1, Decoder1>(isSystematic, kl, sSize, pl, per, iterations, seed, pruneZeros, nl);
	else if ( q == 16 )
        tmpRVal = simMex<Encoder4, Decoder4>(isSystematic, kl, sSize, pl, per, iterations, seed, pruneZeros, nl);
	else if ( q == 256 )
        tmpRVal = simMex<Encoder8, Decoder8>(isSystematic, kl, sSize, pl, per, iterations, seed, pruneZeros, nl);
	else if ( q == 65536 )
        tmpRVal = simMex<Encoder16, Decoder16>(isSystematic, kl, sSize, pl, per, iterations, seed, pruneZeros, nl);
